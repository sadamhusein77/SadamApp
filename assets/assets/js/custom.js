var id_role = 0 // Untuk menampung ID yang kaan di ubah / hapus
$(document).ready(function(){
  ...
  $('#view').on('click', '.btn-form-ubah', function(){ // Ketika tombol dengan class btn-form-ubah pada div view di klik
    id_role = $(this).data('id_role') // Set variabel id dengan id yang kita set pada atribut data-id pada tag button edit
    ...
    var kode_role = tr.find('.kode_role-value').val() // Ambil nis dari input type hidden
    var nama_role = tr.find('.nama_role-value').val() // Ambil nama dari input type hidden
    var akses = tr.find('.akses-value').val() // Ambil alamat dari input type hidden
    $('#kode_role').val(kode_role) // Set value dari textbox nis yang ada di form
    $('#nama_role').val(nama_role) // Set value dari textbox nama yang ada di form
    $('#akses').val(akses) // Set value dari textbox nama yang ada di form
  })
  ...
  $('#view').on('click', '.btn-alert-hapus', function(){ // Ketika tombol dengan class btn-alert-hapus pada div view di klik
    id_role = $(this).data('id_role') // Set variabel id dengan id yang kita set pada atribut data-id pada tag button hapus
  })
  ...
  $('#btn-simpan').click(function(){ // Ketika tombol simpan di klik
    ...
    $.ajax({
      url: base_url + 'admin/simpan_role', // URL tujuan
  ...
  $('#btn-ubah').click(function(){ // Ketika tombol ubah di klik
    ...
    $.ajax({
      url: base_url + 'admin/ubah_role/' + id_role, // URL tujuan
      type: 'POST', // Tentukan type nya POST atau GET
      data: $("#form-modal form").serialize(), // Ambil semua data yang ada didalam tag form
      dataType: 'json',
      beforeSend: function(e) {
  ...
  $('#btn-hapus').click(function(){ // Ketika tombol hapus di klik
    ...
    $.ajax({
      url: base_url + 'admin/hapus_role/' + id_role, // URL tujuan
